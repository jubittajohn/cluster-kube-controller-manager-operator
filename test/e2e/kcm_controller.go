...
const (
	...
	project19922        = "hello-19922"
	infinitePodCreation = "infinite-pod-creation-69072"
)
...
var _ = g.Describe("kube-controller-manager controller workloads", func() {
	...
	g.It("[OTP][OCP-67765] Make sure rolling update logic to exclude unsetting nodes [Serial]", func() {
		...
		ds, err := kubeClient.AppsV1().DaemonSets(ns).Get(context.Background(), "hello-openshift", metav1.GetOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(int(ds.Status.DesiredNumberScheduled)).To(o.Equal(len(nodeList)))

		err = oc.Run("replace").Args("-f", dsUpdate).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		expectedDesired := int32(len(nodeList) - 1)
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
		defer cancel()
		err = wait.PollUntilContextCancel(ctx, 15*time.Second, true, func(ctx context.Context) (bool, error) {
			ds, getErr := kubeClient.AppsV1().DaemonSets(ns).Get(ctx, "hello-openshift", metav1.GetOptions{})
			if getErr != nil {
				return false, nil
			}
			return ds.Status.DesiredNumberScheduled == expectedDesired &&
				ds.Status.NumberAvailable == expectedDesired &&
				ds.Status.UpdatedNumberScheduled == expectedDesired &&
	...
	g.It("[OTP][OCP-69072] Infinite PODs loop creation with NodeAffinity status [Serial]", func() {
		...
		err = oc.WithoutNamespace().Run("create").Args("-f", deployFile, "-n", infinitePodCreation).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		g.DeferCleanup(func() {
			_ = oc.WithoutNamespace().Run("delete").Args("deployment", infinitePodCreation, "-n", infinitePodCreation).Execute()
		})

		testlib.CheckPodStatus(oc, "app="+infinitePodCreation, infinitePodCreation, "Running")
		...
		_, err = oc.WithoutNamespace().Run("patch").Args("deployment", "-n", infinitePodCreation, infinitePodCreation, "--type=merge", "-p", patch).Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		o.Eventually(func() (int, error) {
			podList, err := kubeClient.CoreV1().Pods(infinitePodCreation).List(context.Background(), metav1.ListOptions{
				LabelSelector: "app=" + infinitePodCreation,
			})
			if err != nil {
				return 0, err
			}
			fmt.Fprintf(g.GinkgoWriter, "pod count: %d\n", len(podList.Items))
			for _, pod := range podList.Items {
				if pod.Status.Reason == "NodeAffinity" {
					return len(podList.Items), fmt.Errorf("pod %s has NodeAffinity status", pod.Name)
				}
			}
			return len(podList.Items), nil
		}, 60*time.Second, 10*time.Second).Should(o.BeNumerically(">", 0))
	})
	...
	g.It("[OTP][OCP-19922] Terminating pod should be removed from endpoints list for service", func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.Run("expose").Args("deployment", project19922).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(testlib.WaitForAvailableRsRunning(oc, "deployment", project19922, ns, "1")).To(o.BeTrue())

		podIP, err := oc.Run("get").Args("pod", "-l", "deployment=hello-19922", "-o=jsonpath={.items[*].status.podIP}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())

		err = oc.Run("set").Args("env", "deployment", project19922, "foo=bar").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
		defer cancel()
		err = wait.PollUntilContextCancel(ctx, 10*time.Second, true, func(ctx context.Context) (bool, error) {
			describeSVC, err := oc.Run("describe").Args("svc", project19922).Output()
			if err != nil {
				return false, err
			}
